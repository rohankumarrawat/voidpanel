...
class MigrationParser:
    ...
    def build_system_account(self, auth_password='GeneratedPassword123!'):
        ...
        try:
            ...
            subprocess.run(['sudo', 'chown', '-R', f'{uname}:www-data', f'/home/{uname}/public_html'], check=True)
        ...
        if os.path.exists(source_user_dir):
            try:
                ...
                subprocess.run(['sudo', 'chown', '-R', f'{uname}:www-data', f'/home/{uname}'], check=True)
                subprocess.run(['sudo', 'chmod', '750', f'/home/{uname}'], check=True)
                subprocess.run(['sudo', 'find', f'/home/{uname}', '-type', 'd', '-exec', 'chmod', '750', '{}', '+'], check=True)
                subprocess.run(['sudo', 'find', f'/home/{uname}', '-type', 'f', '-exec', 'chmod', '640', '{}', '+'], check=True)
                # Explicitly allow www-data to write to the logs directory and files inside it
                logs_dir = f'/home/{uname}/logs'
                subprocess.run(['sudo', 'chmod', '770', logs_dir], check=False)
                subprocess.run(['sudo', 'find', logs_dir, '-type', 'f', '-exec', 'chmod', '660', '{}', '+'], check=False)
                for bash_file in ('.bashrc', '.profile', '.bash_profile', '.bash_logout'):
                    bpath = f'/home/{uname}/{bash_file}'
                    if os.path.exists(bpath):
                        subprocess.run(['sudo', 'chmod', '644', bpath], check=True)
