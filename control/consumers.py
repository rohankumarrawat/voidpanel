...
class TerminalConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.pid == 0:
            # Cleanly close all inherited parent file descriptors to prevent socket corruption
            try:
                import resource
...
# Only superusers can connect. Forks a PTY running `su - <user> -s /bin/bash`
...
class UserTerminalConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if self.pid == 0:
            ...
            os.execvp('sudo', ['sudo', 'su', '-', self.target_user, '-s', '/bin/bash'])
