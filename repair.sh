...
# ─────────────────────────────────────────────────────────────────────────────
info "Fix #6 — Creating /var/log/voidpanel with $WEB_USER ownership"
...
chown -R "$WEB_USER:$WEB_USER" /var/log/voidpanel
...
chown "$WEB_USER:$WEB_USER" /var/run/celery
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. Secure Home Directory Permissions (Jail user directory traversal)
# ─────────────────────────────────────────────────────────────────────────────
info "Fixing /home directory permissions for strict user jailing"
chmod 711 /home || true
for hdir in /home/*; do
    if [ -d "$hdir" ] && [ "$(basename "$hdir")" != "vmail" ]; then
        chmod 750 "$hdir" || true
    fi
done
ok "Permissions hardened to 750 on all user home directories"
