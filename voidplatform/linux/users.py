...
class LinuxUserManager(UserManager):
    def create_user(self, username, password, shell='', home_dir=''):
        ...
        result = _run(cmd)
        if result.success:
            # Secure individual home directory (owner and group access only)
            hdir = home_dir or f'/home/{username}'
            _run(['sudo', 'chmod', '751', hdir])
            # Secure /home parent directory (pass-through only, list disabled)
            _run(['sudo', 'chmod', '711', '/home'])
            if password:
