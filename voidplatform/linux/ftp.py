...
class LinuxFTPManager(FTPManager):
    def add_user(self, username, password, home_dir):
        try:
            subprocess.run(f'echo {username} | sudo tee -a {paths.VSFTPD_USERLIST}', shell=True, check=True)
            return self.reload()
        except Exception as e:
    ...
    def remove_user(self, username):
        try:
            subprocess.run(f'sudo sed -i "/^{username}$/d" {paths.VSFTPD_USERLIST}', shell=True, check=True)
            return self.reload()
        except Exception as e:
            return CommandResult(success=False, error=str(e))
