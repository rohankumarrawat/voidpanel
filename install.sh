...
require_ubuntu_22() {
    if ! command -v lsb_release &>/dev/null; then
        die "lsb_release not found. Is this Ubuntu 22.04+?"
    fi
    local NAME VER
    NAME=$(lsb_release -is 2>/dev/null || echo "unknown")
    VER=$(lsb_release -rs  2>/dev/null | cut -d. -f1 || echo "0")
    if [[ "$NAME" != "Ubuntu" || "$VER" -lt 22 ]]; then
        die "VoidPanel requires Ubuntu 22.04 or higher. Detected: $NAME $VER"
    fi
    ok "OS: Ubuntu $VER"
}

# ── Linux distro detection ─────────────────────────────────────────────────────
# Returns: ubuntu | almalinux | rocky | rhel | unsupported
detect_distro() {
    local ID=""
    local VERSION_ID=""
    ...
    case "$ID" in
        ubuntu)
            if [[ "${VERSION_ID:-0}" -lt 22 ]]; then
                die "VoidPanel requires Ubuntu 22.04 or higher. Detected: Ubuntu $VERSION_ID"
            fi
            echo "ubuntu"
            ;;
        almalinux|rocky)
            if [[ "${VERSION_ID:-0}" -lt 8 ]]; then
                die "VoidPanel requires AlmaLinux/Rocky Linux 8 or higher. Detected: $ID $VERSION_ID"
            fi
            echo "rhel"
            ;;
        rhel|centos)
            if [[ "${VERSION_ID:-0}" -lt 8 ]]; then
                die "VoidPanel requires RHEL/CentOS 8 or higher. Detected: $ID $VERSION_ID"
            fi
            echo "rhel"
            ;;
        *)
            die "Unsupported Linux distribution: ${ID:-unknown}. Supported: Ubuntu 22.04+, AlmaLinux 8+, Rocky Linux 8+, RHEL 8+"
            ;;
    esac
}
...
run_linux_install() {
    ...
    if [[ "$DISTRO_FAMILY" == "rhel" ]]; then
        ...
        SCRIPT_URL="https://voidpanel.com/op/almalinux.sh"
    else
        SCRIPT_NAME="ubuntu.sh"
        SCRIPT_URL="https://voidpanel.com/op/ubuntu.sh"
    fi
...
ensure_startup_script() {
    ...
    if curl -fsSL --max-time 30 \
        ...
        "https://voidpanel.com/op/windows-startup.ps1" 2>/dev/null; then
